...
int main() {
    int n, k;
    cin >> n >> k;
    vector <int> elstations(k + 1); 
    vector <vector <int>> ceny(n + 1, vector <int>(n + 1));
    for (int i = 1; i <= k; i++) { cin >> elstations[i]; }
    for (int i = 1; i <= n; i++) { for (int j = 1; j <= n; j++) { cin >> ceny[i][j]; } }
    for (auto i = elstations.begin() + 1; i != elstations.end(); i++) {
        for (auto j = elstations.begin() + 1; j != elstations.end(); j++) { if (*i != *j) { ceny[*i][*j] = ceny[*j][*i] = 0; } }
    }
    int ans = 0;
    while (true) {
